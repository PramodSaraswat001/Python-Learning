...
In this example, `add` is a function that takes a `n1 and n2` argument and returns an addition.
...
Suppose you have a Python file named `module.py`:

```python
# Here calc2 Python file is work as a module
...
## Commands which i learn in this day 
...
pip install virtualenv                 (install virtual envirnment)

python -m venv project-abc             (creating a seperate virtual envirnment with folder name------   project-abc)

source project-abc/bin/activate        (going inside thid venv)

deactive                              (going outside the venv)
